...
${isReferral
  ? `1. Pattern A - 初回メール（流入直後）：紹介経由で初めて連絡するメール。${companyName}の最新ニュースや事業動向に自然に触れ→紹介者の名前は出さず「ご紹介をいただきご連絡いたしました」程度の導入→${companyName}の課題仮説に触れ→サービスの強みを課題に合わせて1-2文で提案→${schedulingUrl ? '日程調整URLへの誘導' : 'アポ打診'}。★第一印象を決める重要なメール。丁寧かつ簡潔に。${contactDepartment ? `特に${contactDepartment}が抱えやすい課題にフォーカスすること。` : ''}
...
4. Pattern D - 掘り起こしメール（1ヶ月後）：1ヶ月前に紹介経由で連絡した前提→「改めてお役に立てることがあるのではと思いご連絡しました」というスタンス→${companyName}の推定される経営課題に具体的に言及→その課題に対して弊社サービスがどう貢献できるかを具体的にアピール→${schedulingUrl ? '日程調整URLへの誘導' : 'メール返信だけでOKと伝える'}→押し売り感を出さず自然な掘り起こしトーンで書くこと。300〜400文字程度。`
  : `1. Pattern A - 初回メール（流入直後）：${leadSourceLabel}直後に送る初回メール。${leadSourceLabel}のお礼→${companyName}の最新ニュースや事業動向に自然に触れ→${companyName}の課題仮説に触れ→サービスの強みを課題に合わせて1-2文で提案→${schedulingUrl ? '日程調整URLへの誘導' : 'アポ打診'}。★第一印象を決める重要なメール。丁寧かつ簡潔に。${contactDepartment ? `特に${contactDepartment}が抱えやすい課題にフォーカスすること。` : ''}導入事例データに同業界or類似課題の事例があれば積極的に活用すること。
