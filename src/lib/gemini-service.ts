...
${serviceDocumentUrl ? `\n★サービス資料URLが設定されています：${serviceDocumentUrl}\n【Pattern A（初回メール＝流入直後）では必ずこの資料リンクを本文に含めてください】\n★重要：Pattern Aは「流入直後」＝初めてのコンタクトです。「先日お届けした資料」「以前お送りした資料」等の既に送付済みかのような表現は絶対に使わないでください。\n初回なので「この度、弊社サービスの詳細をまとめた資料をお送りいたします」「ぜひ下記の資料もご参考ください」等、初めて資料を案内するスタンスで書いてください。\n例：「弊社サービスの詳細をまとめた資料をご用意いたしましたので、ぜひご覧ください。\\n${serviceDocumentUrl}」\nPattern B〜Dでは「先日お送りした資料」と言及してOK（実際にPattern Aで送付済みのため）。` : ''}
...
2. Pattern B - 追撃メール（3日後）：紹介経由で初回連絡してから3日後のフォローアップメール。「先日はお忙しい中、ご確認いただきありがとうございます」等のお礼→初回メールで伝えた内容の補足として同業界の事例を1つ具体的に紹介（企業名はA社等で伏せる）→サービスの強みを課題に合わせて1-2文で提案→${schedulingUrl ? '日程調整URLへの誘導' : 'アポ打診'}。★初回メールの続きとして自然な文脈で書くこと。押しすぎず、追加情報を提供するスタンスで。
...
4. Pattern D - 掘り起こしメール（1ヶ月後）：1ヶ月前に紹介経由で連絡した前提→「改めてお役に立てることがあるのではと思いご連絡しました」というスタンス→${companyName}の推定される経営課題に具体的に言及→その課題に対して弊社サービスがどう貢献できるかを具体的にアピール→${schedulingUrl ? '日程調整URLへの誘導' : 'メール返信だけでOKと伝える'}→押し売り感を出さず自然な掘り起こしトーンで書くこと。300〜400文字程度。`
  : `1. Pattern A - 初回メール（流入直後）：${leadSourceLabel}直後に送る初回メール。${leadSourceLabel}のお礼→${companyName}の最新ニュースや事業動向に自然に触れ→${companyName}の課題仮説に触れ→サービスの強みを課題に合わせて1-2文で提案→${serviceDocumentUrl ? `サービス資料のリンク（${serviceDocumentUrl}）を本文に必ず含め、${leadSourceLabel === '資料ダウンロード' ? '「ダウンロードいただいた資料と合わせて、弊社サービスの詳細資料もぜひご覧ください」等の表現で案内（※「先日お届けした」等の既に送付済みのような表現は禁止。資料DLのお礼→追加資料の案内、という流れで）' : '「弊社サービスの詳細をまとめた資料をご用意いたしましたので、ぜひご覧ください」等の表現で初めて案内するスタンスで自然に案内'}→` : ''}${schedulingUrl ? '日程調整URLへの誘導' : 'アポ打診'}。★第一印象を決める重要なメール。丁寧かつ簡潔に。${contactDepartment ? `特に${contactDepartment}が抱えやすい課題にフォーカスすること。` : ''}導入事例データに同業界or類似課題の事例があれば積極的に活用すること。
2. Pattern B - 追撃メール（3日後）：${leadSourceLabel}から3日後のフォローアップメール。「先日はお忙しい中、資料をご確認いただきありがとうございます」等のお礼→初回メールで伝えた内容の補足として同業界の事例を1つ具体的に紹介（企業名はA社等で伏せる）→サービスの強みを課題に合わせて1-2文で提案→${schedulingUrl ? '日程調整URLへの誘導' : 'アポ打診'}。★初回メールの続きとして自然な文脈で書くこと。押しすぎず、追加情報を提供するスタンスで。
