...
以下の3つのメールを日本語で作成してください：

${isReferral
  ? `1. Pattern A - 追撃メール（3日後）：紹介経由で初回連絡してから3日後のフォローアップメール。「先日はお忙しい中、ご確認いただきありがとうございます」等のお礼→初回メールで伝えた内容の補足として、${companyName}の課題仮説に触れ→同業界の事例を1つ具体的に紹介（企業名はA社等で伏せる）→サービスの強みを課題に合わせて1-2文で提案→${schedulingUrl ? '日程調整URLへの誘導' : 'アポ打診'}。★初回メールの続きとして自然な文脈で書くこと。押しすぎず、追加情報を提供するスタンスで。${contactDepartment ? `特に${contactDepartment}が抱えやすい課題にフォーカスすること。` : ''}
2. Pattern B - 追撃メール（1週間後）：紹介経由で初回連絡してから1週間後のフォローアップメール。「先週ご連絡させていただいた件について、改めてご案内いたします」等の切り出し→${companyName}の最新ニュースや事業動向に触れ→それに関連する課題仮説を提示→弊社サービスで貢献できるポイントを2-3つ箇条書き（サービスの実際の提供内容に基づくこと）→${schedulingUrl ? '日程調整URLへの誘導' : 'アポ打診'}。★Pattern Aとは異なる切り口・異なる課題仮説で書くこと。前回とは違う角度からの提案を強調。
3. Pattern C - 掘り起こしメール（1ヶ月後）：1ヶ月前に紹介経由で連絡した前提→「改めてお役に立てることがあるのではと思いご連絡しました」というスタンス→${companyName}の推定される経営課題に具体的に言及→その課題に対して弊社サービスがどう貢献できるかを具体的にアピール→${schedulingUrl ? '日程調整URLへの誘導' : 'メール返信だけでOKと伝える'}→押し売り感を出さず自然な掘り起こしトーンで書くこと。300〜400文字程度。`
  : `1. Pattern A - 追撃メール（3日後）：${leadSourceLabel}から3日後のフォローアップメール。「先日はお忙しい中、資料をご確認いただきありがとうございます」等のお礼→初回メールで伝えた内容の補足として、${companyName}の課題仮説に触れ→同業界の事例を1つ具体的に紹介（企業名はA社等で伏せる）→サービスの強みを課題に合わせて1-2文で提案→${schedulingUrl ? '日程調整URLへの誘導' : 'アポ打診'}。★初回メールの続きとして自然な文脈で書くこと。押しすぎず、追加情報を提供するスタンスで。${contactDepartment ? `特に${contactDepartment}が抱えやすい課題にフォーカスすること。` : ''}
2. Pattern B - 追撃メール（1週間後）：${leadSourceLabel}から1週間後のフォローアップメール。「先週ご連絡させていただいた件について、改めてご案内いたします」等の切り出し→${companyName}の最新ニュースや事業動向に触れ→それに関連する課題仮説を提示→弊社サービスで貢献できるポイントを2-3つ箇条書き（サービスの実際の提供内容に基づくこと）→${schedulingUrl ? '日程調整URLへの誘導' : 'アポ打診'}。★Pattern Aとは異なる切り口・異なる課題仮説で書くこと。前回とは違う角度からの提案を強調。
3. Pattern C - 掘り起こしメール（1ヶ月後）：${leadSourceLabel}から1ヶ月が経過した前提→「改めてお役に立てることがあるのではと思いご連絡しました」というスタンス→${companyName}の推定される経営課題に具体的に言及→その課題に対して弊社サービスがどう貢献できるかを具体的にアピール→${schedulingUrl ? '日程調整URLへの誘導' : 'メール返信だけでOKと伝える'}→押し売り感を出さず自然な掘り起こしトーンで書くこと。300〜400文字程度。`}
...
- 直近6ヶ月以内（${sixMonthsAgo}以降）のニュースのみ採用すること。それより古い情報は絶対に使わない。dateが${sixMonthsAgo}より前のニュースは出力禁止`;
  ...
  private parseEmailPatterns(
    ...
    const patternNames = [
      { key: 'PATTERN_A', name: '追撃メール（3日後）' },
      { key: 'PATTERN_B', name: '追撃メール（1週間後）' },
      { key: 'PATTERN_C', name: '掘り起こしメール（1ヶ月後）' },
    ];
    ...
    return patterns.length >= 3 ? patterns.slice(0, 3) : patterns;
  }
