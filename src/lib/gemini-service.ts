  ...
  async generateEmails(params: {
    ...
    try {
      const personaInstructions: Record<string, string> = {
        executive: '経営層向けのメール。ROI（投資対効果）と事業成長を前面に。数字や具体的な成果を強調する。',
        manager: '現場責任者向けのメール。業務効率化とプロセス改善に焦点を当てる。運用負荷軽減を強調する。',
        staff:   staff: '担当者向けのメール。個人の業務時間削減と作業効率化に焦点を当てる。日々の工数削減を強調する。',
      };
...
1. Pattern A - 経営層向け（ROI訴求）：CEOやCFO向け。投資対効果と事業成長を前面に。業縮向上への具体的な貢献を示す。
...
この企業について、インサイドシールズの観点から3-4つのキーインサイトを生成してください。各インサイトは1-2文で、アクショナブルで具体的である必要があります。
