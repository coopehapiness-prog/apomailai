...
2. Pattern B - 現場責任者向け（効率化訴求）：部镀長向け。業務効率化とプロセス改善を前面に。チームの生産性向上を示す。
...
2. **動画スクリプト**：LinkedIn動画やYouTube短編用のビジュアル・ノレーション指示。30秒程度。${companyName}と${serviceName}に特化した内容。

3. **フォローアップシノリオ**：メール未返信時のフォローアップメール（3日後、1週間後）と、返信来訪時のメール。各シノリオについて、件名と簡潔な本文を提供。
