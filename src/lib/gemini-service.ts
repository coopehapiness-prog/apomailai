...
${serviceDocumentUrl ? `\n★サービス資料URLが設定されています：${serviceDocumentUrl}\n【Pattern A（初回メール）では必ずこの資料リンクを本文に含めてください】\n自然な形で資料を案内すること。例：「弊社サービスの詳細をまとめた資料をご用意しておりますので、ぜひご覧ください。\\n${serviceDocumentUrl}」\n特にリードソースが「資料ダウンロード」の場合は、「先日ダウンロードいただいた資料に加え、より詳しい資料もご用意しております」等の表現で案内すること。\nPattern B〜Dでも必要に応じて言及してOKだが、毎回入れる必要はない。` : ''}
...
4. Pattern D - 掘り起こしメール（1ヶ月後）：1ヶ月前に紹介経由で連絡した前提→「改めてお役に立てることがあるのではと思いご連絡しました」というスタンス→${companyName}の推定される経営課題に具体的に言及→その課題に対して弊社サービスがどう貢献できるかを具体的にアピール→${schedulingUrl ? '日程調整URLへの誘導' : 'メール返信だけでOKと伝える'}→押し売り感を出さず自然な掘り起こしトーンで書くこと。300〜400文字程度。`
  : `1. Pattern A - 初回メール（流入直後）：${leadSourceLabel}直後に送る初回メール。${leadSourceLabel}のお礼→${companyName}の最新ニュースや事業動向に自然に触れ→${companyName}の課題仮説に触れ→サービスの強みを課題に合わせて1-2文で提案→${serviceDocumentUrl ? `サービス資料のリンク（${serviceDocumentUrl}）を本文に必ず含め、${leadSourceLabel === '資料ダウンロード' ? '「先日ダウンロードいただいた資料に加え、より詳しい資料もご用意しております」等の表現で案内' : '「詳しい資料をご用意しておりますので、ぜひご覧ください」等の表現で自然に案内'}→` : ''}${schedulingUrl ? '日程調整URLへの誘導' : 'アポ打診'}。★第一印象を決める重要なメール。丁寧かつ簡潔に。${contactDepartment ? `特に${contactDepartment}が抱えやすい課題にフォーカスすること。` : ''}導入事例データに同業界or類似課題の事例があれば積極的に活用すること。
