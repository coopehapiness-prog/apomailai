  ...
  const rawPatterns = [
    {
      ...
      body: `${companyName}\nご担当者様\n\nお世話になっております。${senderCompany}の${senderName}です。\n\nこの度は弊社の資料をご請求いただき、誠にありがとうございます。${researchContext}\n\n${industryLabel}企業様とお話しする中で、「${pain1}」が重要な経営テーマになっているケースが多いと感じております。もし貴社でも同様の課題をお持ちでしたら、${serviceIntro}で${serviceBenefit ? serviceBenefit.substring(0, 40) + 'といった改善' : '具体的な改善'}を実現できる可能性がございます。\n\n実際に${industryLabel}A社様では、${serviceIntro}を導入された結果、同様の課題が3ヶ月で改善された事例がございます。\n\n貴社の状況に合わせた具体的な活用イメージもご説明可能ですので、ぜひ一度お話しさせていただければ幸いです。\n\n${ctaBlock}\n\nよろしくお願いいたします。`,
      targetPersona: 'executive',
      description: '初回メール（流入直後）',
    },
    {
      patternName: '追撃メール（3日後）',
      subject: `先日の資料に関する補足のご案内`,
      body: `${companyName}\nご担当者様\n\nお世話になっております。${senderCompany}の${senderName}です。\n\n先日はお忙しい中、弊社の資料をご確認いただきありがとうございます。${researchContext}\n\n本日は補足のご案内としてご連絡いたしました。${industryLabel}企業様とお話しする中で、「${pain1}」が重要な経営テーマになっているケースが多いと伺っております。もし貴社でも同様の課題をお持ちでしたら、${serviceIntro}で${serviceBenefit ? serviceBenefit.substring(0, 40) + 'といった改善' : '具体的な改善'}を実現できる可能性がございます。\n\n実際に${industryLabel}A社様では、${serviceIntro}を導入された結果、同様の課題が3ヶ月で改善された事例がございます。\n\n資料の内容についてご不明点やご質問がございましたら、お気軽にお申し付けください。\n\n${ctaBlock}\n\nよろしくお願いいたします。`,
      targetPersona: 'executive',
      description: '追撃メール（3日後）',
    },
    {
      patternName: '追撃メール（1週間後）',
      subject: `改めてのご案内｜${industry || '業界'}での${serviceName || 'サービス'}活用について`,
      body: `${companyName}\nご担当者様\n\nお世話になっております。${senderCompany}の${senderName}です。\n\n先週ご連絡させていただいた件について、改めてご案内いたします。${researchContext}\n\n前回のご連絡以降、改めて貴社の事業について拝見する中で、弊社サービスが貢献できるポイントを整理いたしました。\n\n・${pain1}に対する${serviceIntro}での具体的な改善\n・${pain2}に関する運用面でのサポート\n・${industryLabel}企業様での導入実績に基づくベストプラクティスの共有\n\n${serviceIntro}は、既存の業務フローを大きく変えることなく導入でき、スピード感のある${industryLabel}企業様にご好評いただいております。\n\n前回とは異なる角度からのご提案も可能ですので、一度お話しさせていただけますと幸いです。\n\n${ctaBlock}\n\nよろしくお願いいたします。`,
      targetPersona: 'manager',
      description: '追撃メール（1週間後）',
    },
    {
      patternName: '掘り起こしメール（1ヶ月後）',
      subject: `改めてのご提案｜${industry || '同業'}業界の最新活用事例`,
      body: `${companyName}\nご担当者様\n\nお世話になっております。${senderCompany}の${senderName}です。\n\n以前ご連絡させていただいた際はお忙しい中ありがとうございました。改めて、貴社のお役に立てることがあるのではと思いご連絡いたしました。\n\n${overviewClean ? `貴社の${overviewClean}について改めて拝見し、` : '貴社の事業について改めて調べさせていただき、'}「${pain1}」という課題に対して、${serviceIntro}で具体的にお力になれる部分があると感じております。\n\n実際に${industryLabel}同規模のB社様では、${serviceIntro}を活用いただいた結果、${pain1}に関連する業務工数が大幅に改善されました。${pain2 !== pain1 ? `また「${pain2}」についても、実績のある改善手法をお持ちしております。` : ''}\n\n以前とは異なる切り口でのご提案も可能ですので、もしよろしければ15〜30分ほどお時間をいただけませんでしょうか。\n\n${ctaBlock}\n\nよろしくお願いいたします。`,
      targetPersona: 'general',
      description: '掘り起こしメール（1ヶ月後）',
    },
  ];
...
export class GeminiService {
  async generateEmails(params: {
    ...
    try {
      ...
      const schedulingUrl = (settings as any).scheduling_url || '';
      // Get service document URL from settings
      const serviceDocumentUrl = (settings as any).service_document_url || '';
...
3. 【同業界の事例のみ使用可】${companyName}と同じ業界の事例がある場合のみ言及する。事例企業名は実名で記載してOK（「A社様」等の匿名化は不要。「株式会社○○様では〜」と実名で紹介する方が説得力が高い）。
4. 【同じ課題の事例のみ使用可】業界は異なるが、${companyName}が抱えていそうな課題と同じ課題を持つ事例がある場合のみ：「貴社と同様に○○の課題をお持ちだった株式会社○○様では〜」と課題の共通点を明示した上で言及する（実名記載OK）。
...
${serviceDocumentUrl ? `\n★サービス資料URLが設定されています：${serviceDocumentUrl}\n【Pattern A（初回メール＝流入直後）では必ずこの資料リンクを本文に含めてください】\n\n★★★ 最重要前提（資料の送付状況）★★★\nPattern Aは「流入直後」に送る最初のメールです。この時点ではリードはまだ資料を受け取っていません。\n- 「資料ダウンロード」の流入経路であっても、リードはフォームで資料を請求しただけで、実際の資料はまだ届いていない状態です。\n- このPattern Aのメールで「初めて」資料を送ります。\n- したがって以下の表現は全て禁止です：\n  ✗「ダウンロードいただいた資料と合わせて」\n  ✗「先日お届けした資料に加え」\n  ✗「以前お送りした資料」\n  ✗「お手元の資料と合わせて」\n  ✗「弊社サービスの詳細をまとめた資料をお送りいたします」＋「資料のご請求ありがとうございます」の両方を使う（重複禁止）\n- ★資料案内は本文中で【1回だけ】記載すること。複数回の資料案内は絶対禁止。\n- 正しい書き方（以下から1つだけ選んで使う）：\n  「ご請求いただいた資料をお送りいたします。ぜひご覧ください。\\n${serviceDocumentUrl}」\n\nPattern B〜Dでは「先日お送りした資料」と言及してOK（Pattern Aで送付済みのため）。` : ''}
...
3. 【課題仮説→ニーズマッチング（最重要）】
   ...
   - OK例：「同業界の株式会社○○様では、弊社サービスの導入により○○が30%改善された事例がございます。」
4. 【柔らかい表現・断定を避ける】
...
${isReferral
  ? `1. Pattern A - 初回メール（流入直後）：紹介経由で初めて連絡するメール。${companyName}の最新ニュースや事業動向に自然に触れ→紹介者の名前は出さず「ご紹介をいただきご連絡いたしました」程度の導入→${companyName}の課題仮説に触れ→サービスの強みを課題に合わせて1-2文で提案→${serviceDocumentUrl ? `サービス資料のリンク（${serviceDocumentUrl}）を本文に必ず含め、「詳しい資料をご用意しておりますので、ぜひご覧ください」等の表現で自然に案内→` : ''}${schedulingUrl ? '日程調整URLへの誘導' : 'アポ打診'}。★第一印象を決める重要なメール。丁寧かつ簡潔に。${contactDepartment ? `特に${contactDepartment}が抱えやすい課題にフォーカスすること。` : ''}
2. Pattern B - 追撃メール（3日後）：紹介経由で初回連絡してから3日後のフォローアップメール。「先日はお忙しい中、ご確認いただきありがとうございます」等のお礼→初回メールで伝えた内容の補足として同業界の事例を1つ具体的に紹介（導入事例に実名がある場合はそのまま実名で紹介）→サービスの強みを課題に合わせて1-2文で提案→${schedulingUrl ? '日程調整URLへの誘導' : 'アポ打診'}。★初回メールの続きとして自然な文脈で書くこと。押しすぎず、追加情報を提供するスタンスで。
3. Pattern C - 追撃メール（1週間後）：紹介経由で初回連絡してから1週間後のフォローアップメール。「先週ご連絡させていただいた件について、改めてご案内いたします」等の切り出し→${companyName}の最新ニュースや事業動向に触れ→それに関連する課題仮説を提示→弊社サービスで貢献できるポイントを2-3つ箇条書き（サービスの実際の提供内容に基づくこと）→${schedulingUrl ? '日程調整URLへの誘導' : 'アポ打診'}。★Pattern Bとは異なる切り口・異なる課題仮説で書くこと。前回とは違う角度からの提案を強調。
4. Pattern D - 掘り起こしメール（1ヶ月後）：1ヶ月前に紹介経由で連絡した前提→「改めてお役に立てることがあるのではと思いご連絡しました」というスタンス→${companyName}の推定される経営課題に具体的に言及→その課題に対して弊社サービスがどう貢献できるかを具体的にアピール→${schedulingUrl ? '日程調整URLへの誘導' : 'メール返信だけでOKと伝える'}→押し売り感を出さず自然な掘り起こしトーンで書くこと。300〜400文字程度。★「その後の状況はいかがでしょうか」「ご検討の状況はいかがでしょうか」等の定型的で受動的な表現は使用禁止。代わりに、新しい情報や具体的な提案を切り口にして能動的にアプローチすること。`
  : `1. Pattern A - 初回メール（流入直後）：${leadSourceLabel}直後に送る初回メール。${leadSourceLabel}のお礼→${companyName}の最新ニュースや事業動向に自然に触れ→${companyName}の課題仮説に触れ→サービスの強みを課題に合わせて1-2文で提案→${serviceDocumentUrl ? `サービス資料のリンク（${serviceDocumentUrl}）を本文に必ず含め、${leadSourceLabel === '資料ダウンロード' ? '「資料のご請求ありがとうございます。下記より資料をご覧いただけます」等の表現で案内（※この時点ではリードはまだ資料を受け取っていない。このメールで初めて送る。「ダウンロードいただいた資料と合わせて」等の既に持っている前提の表現は絶対禁止）' : '「弊社サービスの詳細をまとめた資料をご用意いたしましたので、ぜひご覧ください」等の表現で初めて案内するスタンスで自然に案内'}→` : ''}${schedulingUrl ? '日程調整URLへの誘導' : 'アポ打診'}。★第一印象を決める重要なメール。丁寧かつ簡潔に。${contactDepartment ? `特に${contactDepartment}が抱えやすい課題にフォーカスすること。` : ''}導入事例データに同業界or類似課題の事例があれば積極的に活用すること。
2. Pattern B - 追撃メール（3日後）：${leadSourceLabel}から3日後のフォローアップメール。「先日はお忙しい中、資料をご確認いただきありがとうございます」等のお礼→初回メールで伝えた内容の補足として同業界の事例を1つ具体的に紹介（導入事例に実名がある場合はそのまま実名で紹介）→サービスの強みを課題に合わせて1-2文で提案→${schedulingUrl ? '日程調整URLへの誘導' : 'アポ打診'}。★初回メールの続きとして自然な文脈で書くこと。押しすぎず、追加情報を提供するスタンスで。
3. Pattern C - 追撃メール（1週間後）：${leadSourceLabel}から1週間後のフォローアップメール。「先週ご連絡させていただいた件について、改めてご案内いたします」等の切り出し→${companyName}の最新ニュースや事業動向に触れ→それに関連する課題仮説を提示→弊社サービスで貢献できるポイントを2-3つ箇条書き（サービスの実際の提供内容に基づくこと）→${schedulingUrl ? '日程調整URLへの誘導' : 'アポ打診'}。★Pattern Bとは異なる切り口・異なる課題仮説で書くこと。前回とは違う角度からの提案を強調。
4. Pattern D - 掘り起こしメール（1ヶ月後）：${leadSourceLabel}から1ヶ月が経過した前提→「改めてお役に立てることがあるのではと思いご連絡しました」というスタンス→${companyName}の推定される経営課題に具体的に言及→その課題に対して弊社サービスがどう貢献できるかを具体的にアピール→${schedulingUrl ? '日程調整URLへの誘導' : 'メール返信だけでOKと伝える'}→押し売り感を出さず自然な掘り起こしトーンで書くこと。300〜400文字程度。★「その後の状況はいかがでしょうか」「ご検討の状況はいかがでしょうか」等の定型的で受動的な表現は使用禁止。代わりに、新しい情報や具体的な提案を切り口にして能動的にアプローチすること。`}
...
7. 【導入事例の活用（厳格ルール）】上記に導入事例データがある場合、以下の条件を厳守すること：
   - 【条件1】同じ業界（${(research as any).industry || '不明'}）の事例のみ使用可：業界一致を明示して言及。事例企業は実名で記載OK
   - 【条件2】同じ課題を持つ事例のみ使用可：「貴社と同様に○○の課題をお持ちだった株式会社○○様では〜」と課題の共通点を明示して言及（実名OK）
...
8. 【サービス範囲の厳守（最重要）】提案内容は必ず上記「提供サービス」「サービス概要」「サービスの強み」に記載された範囲内に限定すること。サービスが提供していない機能・領域（例：サプライチェーン管理、サステナビリティ報告、データ分析プラットフォーム等）を勝手に提案するのは絶対NG。
   ...
   - 導入事例の数値（○○%削減、○○時間短縮等）を捏造しないこと。言及する場合は「同業界の株式会社○○様では〜といった改善が見られました」程度の柔らかい表現に留めること
...
本文：1ヶ月前のコンタクトを自然に振り返る（押し売り感を出さない）→「改めてお役に立てることがあるのではと思いご連絡しました」というスタンス→${companyName}の推定される経営課題に具体的に言及→その課題に対して${serviceName || '弊社サービス'}がどう貢献できるかを具体的にアピール（サービスの実際の提供内容に基づくこと）→${schedulingUrl ? `日程調整URL（${schedulingUrl}）への誘導` : 'メール返信だけでOKと伝える'}→300〜400文字程度。★「その後の状況はいかがでしょうか」「ご検討の状況はいかがでしょうか」等の定型表現は禁止。新しい切り口で能動的にアプローチすること。
...
- 【IR・決算ニュース必須ルール】7件のうち必ず1件はIR情報・決算発表・業績報告・有価証券報告書・四半期決算・中期経営計画・株主総会に関するニュースを含めてください。例：「2026年3月期 第3四半期決算発表」「中期経営計画2026-2028を発表」「通期業績予想の上方修正」など。上場企業の場合はIR関連の検索結果から、非上場企業の場合は事業報告・資金調達・経営指標に関するニュースで代替してください。この1件は7件目（最後）に配置してください。
...

{
  ...
  "homepage_url": "企業の会社概要ページの直接URL（/company, /about, /about#company等を優先。ルートURLではなく具体的なページURLを記載。不明な場合は空文字\"\"）",
  "business_url": "企業の事業・サービス・製品紹介ページの直接URL（/service, /products等のページURL。不明な場合は空文字\"\"）",
...
- 直近6ヶ月以内（${sixMonthsAgo}以降）のニュースのみ採用すること。それより古い情報は絶対に使わない。dateが${sixMonthsAgo}より前のニュースは出力禁止`;
  ...
  private parseEmailPatterns(
    ...
    const patternNames = [
      { key: 'PATTERN_A', name: '初回メール（流入直後）' },
      { key: 'PATTERN_B', name: '追撃メール（3日後）' },
      { key: 'PATTERN_C', name: '追撃メール（1週間後）' },
      { key: 'PATTERN_D', name: '掘り起こしメール（1ヶ月後）' },
    ];
  ...
  private getLegacyFallbackPatterns(): EmailPattern[] {
    return [
      {
        patternName: '初回メール（流入直後）',
        subject: '資料ご請求のお礼とご案内',
        body: 'お世話になっております。\n\nこの度は弊社の資料をご請求いただき、誠にありがとうございます。\n\n多くの業界企業様で導入実績がある当サービスは、業務効率化を通じた売上拡大に貢献いたします。\n\n貴社の状況に合わせた具体的な活用イメージもご説明可能ですので、ぜひ一度お話しさせていただければ幸いです。\n\nよろしくお願いいたします。',
        targetPersona: 'executive',
      },
      {
        patternName: '追撃メール（3日後）',
        subject: '先日の資料に関する補足のご案内',
        body: 'お世話になっております。\n\n先日はお忙しい中、弊社の資料をご確認いただきありがとうございます。\n\n本日は補足のご案内としてご連絡いたしました。多くの業界企業様で導入実績がある当サービスは、業務効率化を通じた売上拡大に貢献いたします。\n\n資料の内容についてご不明点がございましたら、お気軽にお申し付けください。\n\nよろしくお願いいたします。',
        targetPersona: 'executive',
      },
      {
        patternName: '追撃メール（1週間後）',
        subject: '改めてのご案内',
        body: 'お世話になっております。\n\n先週ご連絡させていただいた件について、改めてご案内いたします。\n\n当サービスは、現場の業務フローを大きく変えることなく導入でき、スピード感のある企業様にご好評いただいております。\n\n前回とは異なる角度からのご提案も可能ですので、一度お話しさせていただけますと幸いです。\n\nよろしくお願いいたします。',
        targetPersona: 'manager',
      },
      {
        patternName: '掘り起こしメール（1ヶ月後）',
        subject: '改めてのご提案',
        body: 'お世話になっております。\n\n以前ご連絡させていただいた際はお忙しい中ありがとうございました。\n改めて、貴社のお役に立てることがあるのではと思いご連絡いたしました。\n\n業務効率化の観点で、具体的にお力になれる部分があると感じております。\n\nもしよろしければ、15〜30分ほどお時間をいただけませんでしょうか。\n\nよろしくお願いいたします。',
        targetPersona: 'general',
      },
    ];
  }
