  ...
  async generateEmails(params: {
    ...
    try {
      ...
      const personaInstructions: Record<string, string> = {
        ...
        manager:
          '現場責任者向けのメール。業務効率化とプロセス㙲善に焦点を当てる。運用負荷軽減を強調する。',
        staff:
          '担当者向けのメール。個人の業瘩時間削減と作業効率化に焦点を当てる。日々の工数削減を強調する。',
      };
      ...
      const prompt = `あなたは日本のインサイドセールズ(IS)メール作成の専門家です。高度に個別化されたセールスメールを作成します。
